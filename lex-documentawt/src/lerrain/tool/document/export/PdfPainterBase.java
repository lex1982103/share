...
public class PdfPainterBase
{
	public void draw(LexDocument doc, OutputStream os) throws DocumentExportException
	{
		draw(doc, os, new HashMap());
	}

	public void draw(LexDocument doc, OutputStream os, Map appendMap) throws DocumentExportException
	{
		...
		try
		{
			...
			for (int i = 0; i < pageNum; i++)
			{
				...
				for (int j = 0; j < eNum; j++)
				{
					...
					translateElement(pdfContentByte, document, page.getPaper(), i,0, 0, element, appendMap);
				}
			}
		}
		catch (Exception e)
	...
	private void translateElement(PdfContentByte pdf, Document document, Paper paper, int pageNum, int x, int y, LexElement element, Map appendMap) throws DocumentException, MalformedURLException, IOException
	{
...
//			System.out.println(text + " - " + (sh - (y1 - y2)) / 2);
			...
			//colText.setAdjustFirstLine(false);
			//colText.setSimpleColumn(new Phrase(text, font), sx, sy, sx + sw, sy + sh, (sh - (y1 - y2)) / 2 + y1, align);
			if (align == Element.ALIGN_LEFT)
				colText.showTextAligned(pdf, align, new Phrase(text, font), sx, sy + (sh - (y1 - y2)) / 2, 0);
			else if (align == Element.ALIGN_RIGHT)
				colText.showTextAligned(pdf, align, new Phrase(text, font),sx + sw, sy + (sh - (y1 - y2)) / 2, 0);
			else if (align == Element.ALIGN_CENTER)
				colText.showTextAligned(pdf, align, new Phrase(text, font),sx + sw / 2, sy + (sh - (y1 - y2)) / 2, 0);
			colText.go();
...
//				pdf.showTextAligned(align, text, sx + sw / 2, sy + (sh - (y1 - y2)) / 2, 0);
		...
		else if (element instanceof DocumentPanel)
		{
			...
			for (int i = 0; i < count; i++)
				translateElement(pdf, document, paper, pageNum, x + dPanel.getX(), y + dPanel.getY(), dPanel.getElement(i), appendMap);
