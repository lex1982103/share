...
public class DdsV4 implements DataSource
{
...
//			return new DataRecord(null);
	...
	public String[] getGroupsName()
	{
		...
		try
		{
			...
			if (version != 4)
				throw new Exception("version incorrect! this version is " + version + "; this parser is DDS-V4");
	...
	private DataRecord loadFromFile(Factors param, String name) throws Exception
	{
		...
		try
		{
			...
			if (var == null)
			{
				...
				//每行数据数量
				var.setNumberPerLine(dis.readByte());
				//缩放比例
...
//					result.set(k, y, (BigDecimal)d);
		...
		{
			System.out.println("数据未查询到 - " + e.getMessage());
			throw e;
		}
		finally
		{
