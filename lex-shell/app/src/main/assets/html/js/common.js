...
common.url = function(url) {
   ...
   return "http://114.112.96.30:10003/" + url;
...
common.customer = function (msg) {
    var reg = new RegExp("(^|&)" + msg + "=([^&]*)(&|$)"); //构造一个含有目标参数的正则表达式对象
    var r = window.location.search.split('?')[1].match(reg);  //匹配目标参数
    if (r != null) return JSON.parse(decodeURI(r[2])); return {}; //返回参数值
}
