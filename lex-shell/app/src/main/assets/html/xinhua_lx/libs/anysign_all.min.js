/*
 * BeiJing Certificate Authority(BJCA) 2017-11-03 All Right Reserved
 * @author caolili
 *
 * Core function implementations
 * Version Code: miniCore 2.3.1
 */
...
function anysignCommentCore(a, b, c, d) {
	function f() {
		if (I) {
			...
			a.height = I.height / d;
			var h = 1;
			c.mass_word_width / c.mass_word_height <= a.width / a.height ? c.mass_word_width <= a.width && (h = c.mass_word_width / a.width) : c.singleHeight <= a.height && (h = c.mass_word_height / a.height);
			a.width *= h;
			a.height *= h;
			b.scale(1 / d * h, 1 / d * h);
	...
	function g() {
		function b(a) {
			a < e ? (f >= c.mass_words_in_single_line && (u += c.mass_word_height, f = 0), h.drawImage(J[a], c.mass_word_width * f, u, c.mass_word_width, c.mass_word_height), f++, b(a + 1)) : L.push(d.toDataURL("image/png", 1))
		}
	...
	D.innerHTML = v[0].replace(v[0], '<font color="' + c.currentEditBarTextColor + '">$&</font>') + a.substring(1, v.length);
	...
	x.shadowBlur = 5;
	S.onclick = function() {
		p || (n != v.length ? custom_alert("请完成签名", "确认") : (g(), signResCallback && signResCallback(c.cid, L[0].substring(22, L[0].length), L[0].substring(22, L[0].length), H, K, N, O), e()))
	};
	T.onclick = function() {
		if (!p) if (0 >= n) alert("已移除完，请重新进行签名！");
		else {
			...
			d.innerHTML = a;
			d.style.color = "black";
			d.innerHTML = a.substring(0, n) + v[n].replace(v[n], '<font color="' + c.currentEditBarTextColor + '">$&</font>') + a.substring(n + 1, v.length)
		}
	};
	U.onclick = function() {
	...
	"ontouchstart" in document.documentElement ? (A.ontouchstart = function(a) {
		...
		if (l) {
			...
			"undefined" !== typeof a.targetTouches ? (m = Math.floor(a.targetTouches[0].pageX - c.left), u = Math.floor(a.targetTouches[0].pageY - c.top)) : (m = Math.floor(a.pageX - c.left), u = Math.floor(a.pageY - c.top));
			c = a.timeStamp;
			0 !== K || isNaN(c) || (F = c);
			0 < m && u <= A.height && (H = isNaN(c) ? H + (m + "," + u + "," + k + ",0\n") : H + (m + "," + u + "," + k + "," + (c - F) + "\n"), K += 1, m > q && (q = m), m < w && (w = m), u > h && (h = u), u < r && (r = u), y.push({
				x: m,
				y: u
			}), R(), E.push({
				x: m,
		...
		if (l) {
			...
			n >= v.length ? (alert("已签署完毕！"), e.innerHTML = "", p = !1, x.clearRect(0, 0, A.width, A.height)) : P && (P = !1, C = setInterval(function() {
				B.push("-1", "-1");
				if (c.isdistinguish && !checkText(v[n])) showProgress("正在识别，请稍候。。。"), (new OCRObj(c.ocrCapture)).recognition(B, v[n], function(b, m) {
					closeWindow();
					if (0 == b) {
						if (m) {
							n++;
							n == v.length ? (e.innerHTML = "", D.innerHTML = "已签署完毕！", D.style.color = "Red", clearInterval(C)) : (c.isShowBgText && (e.innerHTML = v[n]), D.innerHTML = a, D.innerHTML = a.substring(0, n) + v[n].replace(v[n], '<font color="' + c.currentEditBarTextColor + '">$&</font>') + a.substring(n + 1, v.length));
							var g = document.createElement("img");
							g.id = "img" + n;
							g.width = c.mass_word_width;
							g.height = c.mass_word_height;
							if (checkText(v[n - 1])) g.src = u.toDataURL("image/png");
							else {
								var y = q - w,
									k = h - r,
									l = (q + w) / 2,
									p = (h + r) / 2,
									y = 20 > y && 20 > k ? c.mass_word_width : y > k ? y / 2 : k / 2;
								I = A.getContext("2d").getImageData(l - y - 10, p - y - 10, 2 * (y + 10), 2 * (y + 10));
								l = f();
								g.src = l
							}
							J[n - 1] = g;
					...
					} else x.clearRect(0, 0, A.width, A.height), r = w = 9999, h = q = 0, B = [], clearInterval(C);
					null == d ? console.log("the identify callback is not definition") : "请求超时" != b && d(b)
				});
				else {
					n++;
					B = [];
					...
					if (checkText(v[n - 1])) b.src = u.toDataURL("image/png");
						...
						var m = q - w,
							...
							y = (q + w) / 2,
							k = (h + r) / 2,
							m = 20 > m && 20 > g ? c.mass_word_width : m > g ? m / 2 : g / 2;
						I = A.getContext("2d").getImageData(y - m - 10, k - m - 10, 2 * (m + 10), 2 * (m + 10));
						y = f();
						b.src = y
					}
					J[n - 1] = b;
		...
		y = [];
		E = []
	}, A.ontouchcancel = A.ontouchend) : (A.onmousedown = function(a) {
		null != C && clearInterval(C);
		p = l = !0
		...
		if (l) {
			...
			"undefined" !== typeof a.targetTouches ? (m = Math.floor(a.targetTouches[0].clientX - c.left), u = Math.floor(a.targetTouches[0].clientY - c.top)) : (m = Math.floor(a.clientX - c.left), u = Math.floor(a.clientY - c.top));
			c = a.timeStamp;
			0 !== K || isNaN(c) || (F = c);
			0 < m && (H = isNaN(c) ? H + (m + "," + u + "," + k + ",0\n") : H + (m + "," + u + "," + k + "," + (c - F) + "\n"), K += 1, u += b, m += d, m > q && (q = m), m < w && (w = m), u > h && (h = u), u < r && (r = u), y.push({
				x: m,
				y: u
			}), R(), E.push({
				x: m,
		...
		if (l) {
			...
			"undefined" !== typeof b.targetTouches ? (m = Math.floor(b.targetTouches[0].clientX - e.left), u = Math.floor(b.targetTouches[0].clientY - e.top)) : (m = Math.floor(b.clientX - e.left), u = Math.floor(b.clientY - e.top));
			b = b.timeStamp;
			H = isNaN(b) ? H + "0,0,-1,0\n" : H + ("0,0,-1," + (b - F) + "\n");
			K += 1;
			B.push("-1", "0");
			var g = document.getElementById("comment_canvas");
			g.getContext("2d").drawImage(A, 0, 0);
			var k = document.getElementById("signTitle");
			n >= v.length ? (alert("已签署完毕！"), k.innerHTML = "", x.clearRect(0, 0, A.width, A.height), p = !1) : 10 < E.length && (C = setInterval(function() {
				B.push("-1", "-1");
				if (c.isdistinguish && !checkText(v[n])) showProgress("正在识别，请稍候。。。"), (new OCRObj(c.ocrCapture)).recognition(B, v[n], function(b, u) {
					closeWindow();
					if (0 == b) {
						if (u) {
							n++;
							n == v.length ? (k.innerHTML = "", D.innerHTML = "已签署完毕！", D.style.color = "Red", clearInterval(C)) : (c.isShowBgText && (k.innerHTML = v[n]), D.innerHTML = a, D.innerHTML = a.substring(0, n) + v[n].replace(v[n], '<font color="' + c.currentEditBarTextColor + '">$&</font>') + a.substring(n + 1, v.length));
							var e = document.createElement("img");
							e.id = "img" + n;
							e.width = c.mass_word_width;
							e.height = c.mass_word_height;
							if (checkText(v[n - 1])) e.src = g.toDataURL("image/png");
							else {
								var m = q - w,
									y = h - r,
									l = (q + w) / 2,
									p = (h + r) / 2,
									m = 20 > m && 20 > y ? c.mass_word_width : m > y ? m / 2 : y / 2;
								I = A.getContext("2d").getImageData(l - m - 10, p - m - 10, 2 * (m + 10), 2 * (m + 10));
								l = f();
								e.src = l
							}
							J[n - 1] = e;
					...
					b.width = c.mass_word_width;
					b.height = c.mass_word_height;
					if (checkText(v[n - 1])) b.src = g.toDataURL("image/png");
					else {
						var u = q - w,
							e = h - r,
							m = (q + w) / 2,
							y = (h + r) / 2,
							u = 20 > u && 20 > e ? c.mass_word_width : u > e ? u / 2 : e / 2;
						I = A.getContext("2d").getImageData(m - u - 10, y - u - 10, 2 * (u + 10), 2 * (u + 10));
						m = f();
						b.src = m
					}
					J[n - 1] = b;
	...
	var R = function() {
			if (3 > y.length) {
				...
				for (a = 1; a < y.length - 2; a++) x.quadraticCurveTo(y[a].x, y[a].y, (y[a].x + y[a + 1].x) / 2, (y[a].y + y[a + 1].y) / 2);
				x.stroke()
			}
			P = !0
		},
...
function anysignWebImpl() {
	...
	this._addDataObj = function(a, b) {
		var d;
		if (d = !c) d = 50 <= a && 59 >= a || 500 <= a && 599 >= a || 10 <= a && 19 >= a ? !0 : !1;
		return d ? (b.cid = a, w[a] = b, d = new DataObj, d.Format = b.format, d.Name = b.name, d.Cid = a, p[a] = d, !0) : !1
	};
	this._addSignatureObj = function(a, b) {
		var d;
		if (d = !c && b) d = 20 <= a && 29 >= a || 200 <= a && 299 >= a ? !0 : !1;
		return d ? (b.cid = a, l[a] = b, d = new SignatureObj, d.Cid = a, d.Signer = b.signer, d.SignRule = b.signRule, d.IsTSS = b.isTSS, d.TimeTag = b.timeTag, k[a] = d, !0) : !1
	};
	this._addCommentObj = function(a, b) {
		var d;
		if (d = !c && b) d = 30 <= a && 39 >= a || 300 <= a && 399 >= a ? !0 : !1;
		return d ? (b.cid = a, l[a] = b, d = new MassSignObj, d.Cid = a, d.Signer = b.signer, d.SignRule = b.signRule, d.IsTSS = b.isTSS, k[a] = d, !0) : !1
	};
	this._addEvidenceHash = function(a, b, c, d, f) {
		a = k[a];
		...
		if (d == BioType.PHOTO_SIGNER_IDENTITY_CARD_BACK || d == BioType.PHOTO_SIGNER_IDENTITY_CARD_FRONT) {
			...
			e.Hash = b;
			e.BioType = d;
			a.Points.CertOID.BioFeature.PhotoArray[f] = e;
			return !0
		}
		return d == BioType.SOUND_SIGNER_RETELL || d == BioType.SOUND_SIGNER_OTHER ? (e = new SoundEvidence, e.Format = c, e.Hash = b, e.BioType = d, a.Points.CertOID.BioFeature.SoundArray[f] = e, !0) : !1
	};
	this._addEvidence = function(a, b, c, d, f) {
		a = k[a];
		a.Points || (a.Points = new PlainData);
		a.Points.P10Data || (a.Points.P10Data = new P10Data);
		a.Points.CertOID || (a.Points.CertOID = new CertOID);
		...
		return d == BioType.SOUND_SIGNER_RETELL || d == BioType.SOUND_SIGNER_OTHER ? (e = new SoundEvidence, e.Format = c, b = sha1Digest(b), e.Hash = "BS64:" + b.toString().toUpperCase(), e.BioType = d, a.Points.CertOID.BioFeature.SoundArray[f] = e, !0) : !1
	};
	this._addEvidenceV2 = function(a, b, c, d) {
		...
		return c == BioType.SOUND_SIGNER_RETELL || c == BioType.SOUND_SIGNER_OTHER ? (e = new SoundEvidence, e.Format = DataFormat.MEDIA_3GP, b = sha1Digest(b), e.Hash = "BS64:" + b.toString().toUpperCase(), e.BioType = c, a.Points.CertOID.BioFeature.SoundArray[d] = e, !0) : !1
	};
	...
	this._setTemplate = function(a, b, c, g) {
		if (!d && 10 <= a && 19 >= a) {
			if (null == b || 0 === b.length) return anysign.logger.e("contentUtf8Str must not be null or empty"), !1;
			if (null == c || 0 === c.length) return anysign.logger.e("businessId must not be null or empty"), !1;
			if (null == g || 0 === g.length) return anysign.logger.e("template_serial must not be null or empty"), !1;
			f = c;
			e = sha1DigestBase64(c) + "";
			c = new DataObj;
			c.Cid = a;
			c.Data = b;
			if (10 === a || 13 === a) b = new PDFCrdRule, b.DocFormat = a, b.DocStyleTid = g, c.PDFCrdRule = b;
			g = new DataConfig;
			g.cid = a;
			g.nessesary = !0;
			p[a] = c;
	...
	this._setData = function(a, b) {
		...
		return c && b ? ("string" === typeof b ? (c = p[a], c.Data = Base64.encodeUint8Array(anysign.charset.strToUtf8ArrayUint8(b))) : b instanceof Uint8Array && (c = p[a], c.Data = Base64.encodeUint8Array(b)), !0) : !1
	};
	...
	this._showSignatureDialog = function(a) {
		if (c) {
			if (d) {
				...
				f = e.title;
				h = f.substring(e.titleSpanFromOffset, e.titleSpanToOffset + 1).big();
				a.innerHTML = f.substring(0, e.titleSpanFromOffset) + h + f.substring(e.titleSpanToOffset + 1, f.length);
				setSignResCallback(e, function(a, c, d, f, h, g, u) {
					if (b) {
						if (d) {
							a = k[a];
							var m = e.penColor;
							...
							l.Color = parseInt("0x" + m.substr(1)) + "";
							l.Data = anysign.zip.compressToB64(anysign.charset.strToUint8ArrayAscII(f));
							l.Count = h + "";
							l.Device = new Device;
							...
							l.RefWidth = g;
							l.RefHeight = u;
							a.ImageSize = new ImageSize(calculatedSigWidth, calculatedSigHeight);
							a.Image = c;
							a.SignRule && a.SignRule instanceof SignRule_XYZ && (c = a.SignRule.XYZRule, "dp" === c.Unit ? a.SignRule.XYZRule = {
								Left: c.Left,
								Top: c.Top,
	...
	this._showCommentDialog = function(a) {
		if (c) {
			if (d) {
				...
				var e = l[a];
				setSignResCallback(e, function(a, c, d, f, h, g, m) {
					b && (d ? (a = k[a], a.ImageSize = new ImageSize(g, m), a.Image = c, a.SignRule && a.SignRule instanceof SignRule_XYZ && (c = a.SignRule.XYZRule, "dp" === c.Unit ? a.SignRule.XYZRule = {
						Left: c.Left,
						Top: c.Top,
						Right: c.Left + g,
						Bottom: c.Top - m,
						...
						Pageno: c.Pageno,
						Unit: c.Unit
					})), b(e.cid, CALLBACK_TYPE_COMMENTSIGN, d)) : b(e.cid, CALLBACK_TYPE_DIALOG_CANCEL, null), clear_canvas(), document.getElementById("dialog").style.display = "none", setIsAnysignInputDlgShown(!1))
				});
	...
	this._isReadyToUpload = function() {
		...
		for (var a in l) {
			...
			if (b.nessesary) {
				b = k[e];
				if (null == b) return !1;
				if (isSignOrCom(b.Cid)) {
					if (null == b.Points || null == b.Points.CertOID || null == b.Points.CertOID.BioFeature || null == b.Points.CertOID.BioFeature.Script) return !1
				} else if (null == b.Image) return !1
			}
		}
		for (a in w) if (b = w[a], e = p[b.cid], (null == e || null == e.Data) && b.nessesary) return !1;
		return !0
	};
	this._saveBusinessSession = function(a) {
		...
		b.Channel = g;
		var c, d;
		for (d in l) c = l[d], b.USignConfigs.push(c), b.USignObjs.push(k[c.cid]);
		for (d in w) c = w[d], b.DataConfigs.push(c), b.DataObjs.push(p[c.cid]);
		b.CachetArray = r;
		b.EncAlg = EncAlgType.EncAlg;
		b = anysign.json.stringify(b);
		return tripleDesEncrypt(b, a) + ""
	};
	this._restoreBusinessSession = function(a, b) {
		b = md5Digest("424A4341" + b) + "";
		b = b.substring(0, 24);
		var h = tripleDesDecrypt(a, b) + "",
			h = anysign.hex.hexStrToUint8Str(h),
			h = anysign.json.stringToObj(h);
		f = h.WONo;
		e = h.WOHash;
		g = h.Channel;
		EncAlgType.EncAlg = h.EncAlg;
		var m, q, B;
		for (B in h.USignConfigs) {
			m = h.USignConfigs[B].cid;
			for (var F in h.USignObjs) q = h.USignObjs[F].Cid, m == q && (l[m] = h.USignConfigs[B], k[q] = h.USignObjs[F])
		}
		for (B in h.DataConfigs) for (F in m = h.DataConfigs[B].cid, h.DataObjs) q = h.DataObjs[F].Cid, m == q && (w[m] = h.DataConfigs[B], p[q] = h.DataObjs[F]);
		r = h.CachetArray;
		return d = c = !0
	};
	this._getUploadDataGram = function() {
		if (c && this._isReadyToUpload() && d) {
			var a = new AnySignRoot,
				b = new FormInfo,
				u = new Uint8Array(24);
			void 0 != window.crypto ? window.crypto.getRandomValues(u) : void 0 != window.msCrypto ? window.msCrypto.getRandomValues(u) : capabal.crypto.getRandomValues(u);
			a.EncKey = rsaPubkeyUint8EncByDefault(u);
			a.EncCertSN = encCertSN;
			var q = [],
				E, B;
			for (B in l) E = l[B], (E instanceof SignatureConfig || E instanceof CommentConfig) && "" != k[E.cid].Image && null != k[E.cid].Image && q.push(k[E.cid]);
			var F = [];
	...
	this._getOSInfo = function() {
		...
		return isMobile.iOS() && (a = navigator.userAgent, -1 != (b = a.indexOf("OS")) && -1 != (c = a.indexOf("like Mac OS"))) ? (a = a.substring(b + 3, c - 1), a = a.replace(/_/g, "."), "ios##" + a) : "unknown"
	}
}
var isMobile = {
	Android: function() {
...
function am2(a, b, c, d, f, e) {
	...
	for (b >>= 15; 0 <= --e;) {
		var k = this[a] & 32767,
			l = this[a++] >> 15,
			p = b * k + l * g,
			k = g * k + ((p & 32767) << 15) + c[d] + (f & 1073741823);
...
function bnpFromString(a, b) {
	...
	8 == c && 0 != (a[0] & 128) && (this.s = -1, 0 < e && (this[this.t - 1] |= (1 << this.DB - e) - 1 << e));
	this.clamp();
...
function bnpLShiftTo(a, b) {
	var c = a % this.DB,
		...
		g = this.s << c & this.DM,
		k;
	for (k = this.t - 1; 0 <= k; --k) b[k + e + 1] = this[k] >> d | g, g = (this[k] & f) << c;
		...
		if (f.t < d.t) null != b && b.fromInt(0), null != c && this.copyTo(c);
			...
			if (0 != f) {
				var l = f * (1 << this.F1) + (1 < d ? e[d - 2] >> this.F2 : 0),
					...
					h = null == b ? nbi() : b;
				e.dlShiftTo(q, h);
				0 <= c.compareTo(h) && (c[c.t++] = 1, c.subTo(h, c));
				BigInteger.ONE.dlShiftTo(d, h);
				for (h.subTo(e, e); e.t < d;) e[e.t++] = 0;
				for (; 0 <= --q;) {
					var m = c[--r] == f ? this.DM : Math.floor(c[r] * p + (c[r - 1] + w) * l);
					if ((c[r] += e.am(0, m, c, q, 0, d)) < m) for (e.dlShiftTo(q, h), c.subTo(h, c); c[r] < --m;) c.subTo(h, c)
				}
	...
	Base64 = {
		...
		encodeUint8Array: function(a) {
			for (var b = 0, c = a.length, d = "", f, e = ""; b < c;) {
				...
				for (var g = 0, k = f.length; g < k; g++) e += String.fromCharCode(f[g]);
				d += e;
				b += 32768;
				e = ""
			}
		...
		decode: function(a) {
			...
			for (var l = new ArrayBuffer(b), p = new Uint8Array(l), b = 0; b < c; b += 4) f = lookup[a.charCodeAt(b)], e = lookup[a.charCodeAt(b + 1)], g = lookup[a.charCodeAt(b + 2)], k = lookup[a.charCodeAt(b + 3)], p[d++] = f << 2 | e >> 4, p[d++] = (e & 15) << 4 | g >> 2, p[d++] = (g & 3) << 6 | k & 63;
			return l
		},
		encode: function(a) {
			var b = "",
				c, d, f, e, g, k, l = 0;
			for (a = Base64._utf8_encode(a); l < a.length;) c = a.charCodeAt(l++), d = a.charCodeAt(l++), f = a.charCodeAt(l++), e = c >> 2, c = (c & 3) << 4 | d >> 4, g = (d & 15) << 2 | f >> 6, k = f & 63, isNaN(d) ? g = k = 64 : isNaN(f) && (k = 64), b = b + this._keyStr.charAt(e) + this._keyStr.charAt(c) + this._keyStr.charAt(g) + this._keyStr.charAt(k);
			return b
		},
	...
	anysign = anysign || {
		binary: {
			uint32ArrayToUint8Array: function(a) {
				if (!a) return null;
				for (var b = new Uint8Array(4 * a.length), c, d = 0, f = a.length; d < f; d++) c = d << 2, b[c] = a[d] >> 24 & 255, b[c + 1] = a[d] >> 16 & 255, b[c + 2] = a[d] >> 8 & 255, b[c + 3] = a[d] & 255;
				return b
			}
		},
		hex: {
...
(function(a) {
	function b() {
		for (var b = !0, c, d = w, e = p.length; d < e && b;) if (c = p[d], void 0 !== c && null !== c) {
			if ("string" === typeof c) {
				var f = k.cloneNode(!1);
				f.text = c;
	...
	function c() {
		if (p.length) {
			var b = p.pop(),
				d;
			"string" === typeof b ? (d = k.cloneNode(!0), d.type = "text/javascript", d.async = !0, d.src = b, d.onload = d.onreadystatechange = function() {
				if (!d.readyState || /loaded|complete/.test(d.readyState)) d.onload = d.onreadystatechange = null, d = void 0, c()
	...
	function f() {
		var a = arguments.length,
			c, e;
		for (c = 0; c < a; c += 1)"string" === typeof arguments[c] ? (e = r(arguments[c]), e.onload = d(e, p.length), p[p.length] = null, e.send()) : (p[p.length] = arguments[c], b())
	}
	function e() {
	...
	var g = a.document,
		...
		l = g.getElementsByTagName("script")[0],
		p = [],
		w = 0,
		r, q, h;
	a.XMLHttpRequest && (q = new a.XMLHttpRequest, "withCredentials" in q ? h = function(b) {
		q = new a.XMLHttpRequest;
		q.open("get", b, !0);
		return q
	} : a.XDomainRequest && (h = function(b) {
...
function IsPC() {
	for (var a = navigator.userAgent, b = "Android;iPhone;SymbianOS;Windows Phone;iPad;iPod".split(";"), c = !0, d = 0; d < b.length; d++) if (0 < a.indexOf(b[d])) {
		c = !1;
		break
	}
	return c
	...
	identify_callback, fresh = function() {
		if (isSign) {
			...
			if (1 == a) {
				...
				r.style.height = a + "px";
				r.style.width = h + "px";
				10 > b.width && (b.width *= h);
				b.width < h && (b.width = h);
				b.height = .65 * a;
				h = document.getElementById("container");
				h.style.overflowX = "scroll";
				h.style.overflowY = "hidden";
				h.style.height = .65 * a + "px";
				h = document.getElementById("anysign_title");
				h.style.height = .1 * a + "px";
				document.getElementById("single_scrollbar").style.height = .1 * a + "px";
				h = document.getElementById("btnContainerOuter");
				h.style.height = .1 * a + "px"
			} else 0 == a && (0 == window.orientation || 180 == window.orientation ? (a = .9 * getWindowHeight(), h = getWindowWidth()) : (c = 569 > getWindowWidth() ? .75 * getWindowHeight() : getWindowHeight(), b = getWindowWidth(), a = c, h = b), b = document.getElementById("anysignCanvas"), c = b.getContext("2d"), isDrawn && (d = document.createElement("canvas"), f = d.getContext("2d"), w = p = g = e = l = paste_padding, 0 > (k = minX - l) && (e = minX, k = 0), 0 > (r = minY - l) && (g = minY, r = 0), (q = maxX + l) > b.width && (p = 0, q = b.width), (l = maxY + l) > b.height && (w = 0, l = b.height), e = maxX - minX + e + p, g = maxY - minY + g + w, k = c.getImageData(k, r, q - k, l - r), d.width = e, d.height = g), r = document.getElementById("dialog"), r.style.height = a + "px", r.style.width = h + "px", b.height = a, 10 > b.width && (b.width *= h), b.width < h && (b.width = h), 0 == window.orientation || 180 == window.orientation ? (b.height *= .7, h = document.getElementById("container"), h.style.overflowX = "scroll", h.style.overflowY = "hidden", h.style.height = .7 * a + "px", h = document.getElementById("anysign_title"), h.style.height = .1 * a + "px", h = document.getElementById("btnContainerOuter"), h.style.height = .2 * a + "px", a = document.getElementById("btnContainerInner"), a.style.marginBottom = 0) : (b.height *= .6, h = document.getElementById("container"), h.style.overflowX = "scroll", h.style.overflowY = "hidden", h.style.height = .6 * a + "px", h = document.getElementById("anysign_title"), h.style.height = .1 * a + "px", h = document.getElementById("btnContainerOuter"), h.style.height = .3 * a + "px", a = document.getElementById("btnContainerInner"), a.style.marginBottom = h.style.height));
			...
			isDrawn && (a = b.width > e ? b.height >= g ? 1 : b.height / g : b.width / b.height > e / g ? b.height / g : b.width / e, f.putImageData(k, 0, 0), f = document.createElement("canvas"), h = f.getContext("2d"), f.width = e * a, f.height = g * a, h.scale(a, a), h.drawImage(d, 0, 0), c.clearRect(0, 0, b.width, b.height), c.drawImage(f, 0, 0), minY = minX = 0, maxX *= a, maxY *= a);
	...
	if (document.getElementById("anysignCanvas")) {
		...
		"ontouchstart" in document.documentElement ? (canvas.ontouchstart = function(a) {
			...
			if (isDown && !isCopyingImg) {
				...
				"undefined" !== typeof a.targetTouches ? (curX = Math.floor(a.targetTouches[0].pageX - b.left), curY = Math.floor(a.targetTouches[0].pageY - b.top)) : (curX = Math.floor(a.pageX - b.left), curY = Math.floor(a.pageY - b.top));
				b = a.timeStamp;
				0 !== signTrachPointCount || isNaN(b) || (firstPointTime = b);
				0 < curX && curY <= canvas.height && (signTrack = isNaN(b) ? signTrack + (curX + "," + curY + "," + base_stroke_width + ",0\n") : signTrack + (curX + "," + curY + "," + base_stroke_width + "," + (b - firstPointTime) + "\n"), signTrachPointCount += 1, curX > maxX && (maxX = curX), curX < minX && (minX = curX), curY > maxY && (maxY = curY), curY < minY && (minY = curY), points.push({
					x: curX,
					y: curY
			...
			if (isDown) {
				...
				b = a.timeStamp;
				0 !== signTrachPointCount || isNaN(b) || (firstPointTime = b);
				0 < curX && (signTrack = isNaN(b) ? signTrack + (curX + "," + curY + "," + base_stroke_width + ",0\n") : signTrack + (curX + "," + curY + "," + base_stroke_width + "," + (b - firstPointTime) + "\n"), signTrachPointCount += 1, curY += c, curX += e, curX > maxX && (maxX = curX), curX < minX && (minX = curX), curY > maxY && (maxY = curY), curY < minY && (minY = curY), points.push({
					x: curX,
					y: curY
				}), onPaint(), 0 < curX && 0 < curY && sbuilder.push(curX, curY), lastX = curX, lastY = curY);
				preventDefault(a)
...
function onload_singleSingScrollAction() {
	...
	function b() {
		d += k;
		d < f ? (e.scrollLeft = d, setTimeout(b, g)) : (d = f, e.scrollLeft = d);
...
function sign_confirm() {
	if (isDrawn) if (signObjTmp.isdistinguish) {
		...
		for (var a = "", b = signObjTmp.signer.UName.replace(/(.)(?=[^$])/g, "$1,").split(","), c = 0; c < b.length; c++) checkText(b[c]) || (a += b[c]);
		(new OCRObj(signObjTmp.ocrCapture)).recognition(sbuilder, a, function(a, b) {
			closeWindow();
			if (0 == a) {
				var c = document.getElementById("anysignCanvas");
				imageDataTmp = c.getContext("2d").getImageData(minX - paste_padding, minY - paste_padding, maxX - minX + paste_padding + paste_padding + paste_padding, maxY - minY + paste_padding + paste_padding + paste_padding);
				if (signResCallback) {
					var d = getSigData(),
						c = document.getElementById("anysignCanvas");
					signResCallback(signObjTmp.cid, d[0].substr(22, d[0].length), d[1].substr(22, d[1].length), signTrack, signTrachPointCount, c.width, c.height)
				}
				document.body.parentNode.style.overflow = "scroll";
				isSign = !1
			} else "请求超时" == a ? closeWindow() : null == identify_callback ? console.log("the identify callback is not definition") : identify_callback(a), clear_canvas()
		});
		sbuilder = []
	} else a = document.getElementById("anysignCanvas"), imageDataTmp = a.getContext("2d").getImageData(minX - paste_padding, minY - paste_padding, maxX - minX + paste_padding + paste_padding + paste_padding, maxY - minY + paste_padding + paste_padding + paste_padding), signResCallback && (b = getSigData(), a = document.getElementById("anysignCanvas"), signResCallback(signObjTmp.cid, b[0].substr(22, b[0].length), b[1].substr(22, b[1].length), signTrack, signTrachPointCount, a.width, a.height)), document.body.parentNode.style.overflow = "scroll", isSign = !1;
...
function setSignResCallback(a, b) {
	if (isSignOrCom(a.cid)) {
		...
		var c = document.getElementById("anysignCanvas"),
			c = c.getContext("2d");
		c.strokeStyle = signObjTmp ? signObjTmp.penColor : "black"
	} else commentObjTmp = a, signResCallback = b, c = document.getElementById("comment_canvas"), c = c.getContext("2d"), c.strokeStyle = commentObjTmp ? commentObjTmp.penColor : "black", comment_canvas.strokeStyle = commentObjTmp ? commentObjTmp.penColor : "black"
}
...
function getSigData() {
	...
	if (imageDataTmp) {
		var b = document.createElement("canvas"),
			c = b.getContext("2d"),
			d = window.devicePixelRatio ? window.devicePixelRatio : 1;
		b.width = imageDataTmp.width / d;
		b.height = imageDataTmp.height / d;
		var f = 1;
		signObjTmp.singleWidth / signObjTmp.singleHeight <= b.width / b.height ? signObjTmp.singleWidth <= b.width && (f = signObjTmp.singleWidth / b.width) : signObjTmp.singleHeight <= b.height && (f = signObjTmp.singleHeight / b.height);
		b.width *= f;
		b.height *= f;
		calculatedSigWidth = b.width;
		calculatedSigHeight = b.height;
		c.scale(1 / d * f, 1 / d * f);
		var e = document.createElement("canvas");
		e.width = imageDataTmp.width;
		e.height = imageDataTmp.height;
		e.getContext("2d").putImageData(imageDataTmp, 0, 0);
		c.drawImage(e, 0, 0);
		f = 1 / d * f * signObjTmp.signatureImgRatio;
		c = document.createElement("canvas");
		d = c.getContext("2d");
		1 > f ? (c.width = imageDataTmp.width * f, c.height = imageDataTmp.height * f, d.scale(f, f)) : (c.width = imageDataTmp.width, c.height = imageDataTmp.height);
		f = document.createElement("canvas");
		f.width = imageDataTmp.width;
		f.height = imageDataTmp.height;
		f.getContext("2d").putImageData(imageDataTmp, 0, 0);
		d.drawImage(f, 0, 0);
		a[0] = c.toDataURL();
		a[1] = b.toDataURL();
		return a
...
function testEncAndDec() {
	var a = tripleDesEncrypt("abdfdsafdasfcdef132432432432g你好吗,,,fdafdas", "133434") + "",
		b = tripleDesDecrypt(a, "133434") + "",
		b = anysign.hex.hexStrToUint8Str(b),
		a = "abdfdsafdasfcdef132432432432g你好吗,,,fdafdas\n" + (a + "\n") + (b + "\n");
...
var JQuery_Capable = {
	offset: function(a) {
		var b, c = {
			...
			d = a && a.ownerDocument;
		if (d) return b = d.documentElement, "undefined" !== typeof a.getBoundingClientRect && (c = a.getBoundingClientRect()), a = JQuery_Capable.getWindow(d), {
			top: c.top + (a.pageYOffset || b.scrollTop) - (b.clientTop || 0),
			left: c.left + (a.pageXOffset || b.scrollLeft) - (b.clientLeft || 0)
		}
	},
...
function showMessageBox2(a, b, c, d) {
	...
	c.id = "mesWindow";
	c.innerHTML = "<div id='mesWindowContent'>" + b + "</div><div id='mesWindowBottom'><input id='mesWindowBtnOk' type='button' onclick='closeWindow();'value='确认' /></div>";
	a.appendChild(c);
...
(function() {
	"btoa" in window || (window.btoa = function(a) {
		var b = "",
			c, d;
		c = 0;
		for (d = a.length; c < d; c += 3) var f = a.charCodeAt(c) & 255,
			e = a.charCodeAt(c + 1) & 255,
			g = a.charCodeAt(c + 2) & 255,
			k = (f & 3) << 4 | e >> 4,
			e = c + 1 < d ? (e & 15) << 2 | g >> 6 : 64,
			g = c + 2 < d ? g & 63 : 64,
			b = b + ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(f >> 2) + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(k) + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(e) + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".charAt(g));
		return b
	})
})();
var capabal = capabal || {
...
function adler32(a, b, c) {
	...
	for (var d = 1, f = 0, e = 0; e < c; e++) d = (d + a[b + e]) % 65521, f = (f + d) % 65521;
...
function crc32(a, b, c) {
	...
	if (!d) {
		for (var d = [], f, e = 0; 256 > e; e++) {
			f = e;
			for (var g = 0; 8 > g; g++) f = f & 1 ? 3988292384 ^ f >>> 1 : f >>> 1;
			d[e] = toUInt(f)
		}
	...
	var a = function() {
			...
			for (var g = f * (4 * b + 1), k = 0; k < f; k++) a.splice(k * (4 * b + 1), 0, 0);
			b = Math.ceil(g / 32768);
			Array.prototype.push.apply(e, bytes32(g + 5 * b + 6));
			f = e.length;
			k = g + 5 * b + 6 + 4;
			e.push(73, 68, 65, 84, 120, 1);
			for (var l = 0; l < b; l++) {
				var p = Math.min(32768, g - 32768 * l);
				e.push(l == b - 1 ? 1 : 0);
	...
	HTMLCanvasElement.prototype.toDataURL = function(c) {
		...
		if ("data:," == d) return HTMLCanvasElement.prototype.toDataURL = a, this.toDataURL();
...
function OCRObj(a) {
	...
	c.transID = UUID();
	c.encKey = "encKey";
	null == b.appID || "" == b.appID || void 0 == b.appID ? console.log("ocrCapture.appID is null") : (c.userID = b.appID, null == b.serviceID || "" == b.serviceID || void 0 == b.serviceID ? console.log("ocrCapture.serviceID is null") : (c.serviceID = b.serviceID, d.type = 1, b.language == Language.CHS ? d.lang = "1" : b.language == Language.CHT && (d.lang = "2"), d.similar = b.resolution, d.text = b.text, d.hwrNum = b.count, c.hwrInfo = d, this.recognition = function(a, k, l) {
		if (null == a || "" == a || void 0 == a) return "Identification parameter is empty";
		if (10 > a.length) return "too few coordinates";
		f = l;
		OCRObj();
		null != d && (d.strokes = a, d.hwrNum = b.count, c.hwrInfo = d);
		null == b.IPAdress || "" == b.IPAdress || void 0 == b.IPAdress ? f("服务器地址为空！", e) : jQuery.ajax({
			type: "POST",
			...
			url: b.IPAdress,
			success: function(a) {
				if (null == a || "undefined" == a || "" == a) f("链接服务器异常,请检查网络！", e);
				else {
					a = JSON.parse(a);
					if (null == a || "undefined" == typeof a) alert("服务器连接失败,请稍后再次尝试");
					else if ("0" == a.errCode) for (var b = a.result, c = "", d = 0; d < b.length; d++) if ("0" == b[d]) {
						if (k == c) {
							e = !0;
							break
						}
						c = ""
					} else var g = String.fromCharCode(b[d]),
						c = c + g;
			...
			error: function(a, b, c) {
				f("请求超时", e);
				if ("timeout" == b) alert("请求超时,请稍后再次尝试");
				else return alert("连接服务端异常: " + b + "；status code：" + a.status + "；state：" + a.readyState), a = document.getElementById("comment_canvas"), a.getContext("2d").clearRect(0, 0, a.width, a.height), !1
			}
		})
	}))
}
