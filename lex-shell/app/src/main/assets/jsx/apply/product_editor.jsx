class Main extends React.Component {
    ...
    componentDidMount() {
        this.state.form = [{}]
        APP.apply.editProduct(this.state.planId, null, null, null, this.state.index, r => {
            this.state.form[0] = {
                name: r.name,
        ...
        APP.apply.viewPlan(this.state.planId, plan => {
            APP.apply.listRiders(this.state.planId, null, null, this.state.index, r => {
                r.map(v => {
                    let prdForm = {
                        name: v.name,
                        productId: v.code,
                    ...
                    plan.product.map((r3, i) => {
                        if (r3.productId == v.code && r3.parent == this.state.index) {
                            APP.apply.editProduct(this.state.planId, null, null, null, i, r4 => {
            ...
            APP.apply.addProduct(this.state.planId, this.state.index, productId, r => {
                r.product.map((r2, i) => {
                    if (r2.productId == productId && r2.parent == this.state.index) 
                        APP.apply.editProduct(this.state.planId, null, null, null, i, r1 => {
