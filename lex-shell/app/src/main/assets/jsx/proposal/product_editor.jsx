class Main extends React.Component {
    ...
    componentDidMount() {
        ...
        APP.proposal.viewPlan(this.state.planId, plan => {
            APP.proposal.listRiders(this.state.planId, this.state.index, r => {
                r.map(v => {
                    let prdForm = {
                        name: v.name,
                        productId: v.code,
                    ...
                    plan.product.map((r3, i) => {
                        if (r3.productId == v.code && r3.parent == this.state.index) {
                            APP.proposal.editProduct(this.state.planId, i, r4 => {
    ...
    addRider(prdIndex, prd) {
        ...
        if (riderForm.form == null) {
            ...
            APP.proposal.deleteProduct(this.state.planId, this.state.index, productId, r => {
                riderForm.form = null
                this.setState({ form: this.state.form })
            })
        }
